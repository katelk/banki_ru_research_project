# Влияние платных акций за отзывы на публичный рейтинг банка
...
## Главная находка: платные акции влияют на публичный рейтинг банка
...
├── reviews.csv # исходный список отзывов
├── reviews_clean.csv # очищенный датасет (одна строка = пара отзыв-продукт)
├── reviews_by_bank.csv # дедуплицированный датасет (одна строка = один отзыв)
├── reviews_by_bank.csv # дедуплицированный датасет (одна строка = один отзыв)
...
65 CSV-файлов по парама банк-продукт не включены в репозиторий из-за объёма и получаются только при запуске 1.parsing